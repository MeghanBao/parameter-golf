...
# Module-level QAT flag: avoids torch.compile constant-folding a class attribute.
# torch.compile sees this as a global with a guard and recompiles when it flips True.
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                ...
                v_embed=ve, v0=v0, resid_lambda_attn=rl[i, 0], resid_lambda_mlp=rl[i, 1])
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                v_embed=ve, v0=v0, resid_lambda_attn=rl[bi, 0], resid_lambda_mlp=rl[bi, 1])
        x = self.final_norm(x)
    ...
    def forward_hidden(self, input_ids: Tensor) -> Tensor:
        """Return final hidden states (before lm_head) without computing loss."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
    ...
    def compute_logits(self, hidden_states: Tensor) -> Tensor:
        """Compute logits from final hidden states (output of forward_hidden)."""
        if self.tie_embeddings:
...
def eval_val_sliding_ttt(
    ...
    log0(f"ttt_sliding:start chunks={num_chunks} chunk_tokens={ttt_chunk} "
         ...
         + (f" slot_lr={args.slot_lr} slot_steps={args.slot_steps}" if use_slot else ""))
    ...
    for ci in range(num_chunks):
        ...
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            if use_slot:
                # SLOT: get hidden states (no grad needed through model weights)
                with torch.no_grad():
                    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                        H = base_model.forward_hidden(x_batch)
                H = H.float()
                # Optimize per-batch delta via AdamW (grads only through compute_logits)
                ...
                for _step in range(args.slot_steps):
                    ...
                    slot_loss = F.cross_entropy(
                        adapted_logits.reshape(-1, adapted_logits.size(-1)),
                        y_batch.reshape(-1))
                    slot_loss.backward()
                    slot_opt.step()
                with torch.no_grad():
...
def main() -> None:
    ...
    replicated_params.append(base_model.resid_lambdas)
    ...
    optimizers: list[torch.optim.Optimizer] = [optimizer_tok, optimizer_muon, optimizer_scalar, optimizer_resid_lambdas]
    ...
    log0(
        ...
        f"matrix_lr:{args.matrix_lr} scalar_lr:{args.scalar_lr} resid_lambda_lr:{resid_lambda_lr:.4f}"
    )
    log0(
    ...
    if args.lawa_enabled and len(lawa_queue) > 1:
        ...
        for name in swa_state:
            swa_state[name] = (swa_state[name] / swa_count).to(dtype=current_state[name].dtype)
